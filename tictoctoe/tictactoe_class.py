...
class TicTacToe:
    def __init__(self):
        ...
        if self.Game_status() == 3:
            ...
            print('Player {} has won !'.format(
                ['\033[93mX', '\033[0mO'][self.Game_status() - 1]))

    def show_board(self):
        '''
        show tictactoe board 
        '''
        print(*['{} | {} | {}'.format(*[self.vertebrae[j]
              for j in self.board[i]]) for i in range(3)], sep='\n'+'-'*9+'\n')
    ...
    def Game_status(self):
        ...
        options = [column for column in self.board] \
            ...
            + [[self.board[i][i]
                for i in range(3)]]+[[self.board[i][2-i] for i in range(3)]]
